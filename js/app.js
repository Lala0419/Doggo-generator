...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        const breedsArray = Object.keys(breedsObject) //making it(object) into an array. but why? = we can only grab the data if its an array?
        for (let i=0; i<breedsArray.length;i++){
            ...
            select.appendChild(option) //appending each options to our select drop down box
        }
    ...
    select.addEventListener('change', event => {
        ...
        getDoggoImg(url) //this is a function
        doggoInfo.assignMF()
        doggoInfo.assignAge()
    ...
    const getDoggoImg = url => { //this url is just an parameter
        fetch(url) //going to the API url avobe
        .then(res => {
    ...
    const doggoInfo = {
        ...
        // assinLikesList(){
        //     this.age = array[Math.floor(Math.random() * array.length)]
